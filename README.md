...
# IDM Trial Reset: Quick Setup

**1. Allow Scripts**
Open PowerShell as **Administrator** and run (Type `Y` and press `Enter` if prompted):
...
**2. Install the Tool**
Paste this entire block into PowerShell and press `Enter`. It automatically safely configures your system profile:
...
function reset-idm {
    ...
    foreach ($p in $Processes) {
        if (Get-Process -Name $p -ErrorAction SilentlyContinue) {
            ...
            Write-Host "[OK] Terminated $p" -ForegroundColor Green
        }
    }
    ...
    foreach ($RegPath in $RegPaths) {
        if (Test-Path $RegPath) {
            if ($RegPath -match "DownloadManager") {
                $Keys = @("Serial", "Email", "FName", "LName", "LstCheck", "Trial")
                foreach ($Key in $Keys) {
                    Remove-ItemProperty -Path $RegPath -Name $Key -ErrorAction SilentlyContinue
                }
                Write-Host "[OK] Scrubbed DownloadManager licensing" -ForegroundColor Green
            } else {
                Remove-Item -Path $RegPath -Recurse -Force -ErrorAction SilentlyContinue
                Write-Host "[OK] Nuked CLSID Anchor: $RegPath" -ForegroundColor Green
            }
        }
    }

    $AppDataDir = "$env:APPDATA\IDM"
    if (Test-Path $AppDataDir) {
        Get-ChildItem -Path $AppDataDir -Filter "*.dat" | Remove-Item -Force -ErrorAction SilentlyContinue
        Write-Host "[OK] Local AppData cache cleared" -ForegroundColor Green
    }
...
Set-Content -Path $PROFILE -Value $cleanCode -Encoding UTF8

```

**3. Run It**
Reload your profile to activate the command, then run it:
...
### ⚡ Bonus: 1-Click Desktop Shortcut

Right-click your Desktop -> **New** -> **Shortcut**. Paste this into the location box:
...
Click **Next**, name it "Reset IDM", and click **Finish**.
